...
import type { SOPWorkflow, Category } from '../types';
...
export function SOPManager({ onEdit, onCreate, refreshKey }: SOPManagerProps) {
  ...
  useEffect(() => {
    fetchCategories();
    fetchWorkflows();
  }, [fetchCategories, fetchWorkflows, refreshKey]);
    ...
    <div className="space-y-8 min-h-[600px]">
      ...
      {loading ? (
        ...
        <div className="space-y-12">
          {(() => {
            if (categories.length === 0 && workflows.length === 0) {
              return (
            ...
            // Priority order for specific core categories
            const prioritySlugs = ["market_research", "product_development", "analytics"];
            
            // Group workflows by category_id
            const grouped = workflows.reduce((acc, wf) => {
              const catId = wf.category_id || 0;
              if (!acc[catId]) acc[catId] = [];
              acc[catId].push(wf);
              return acc;
            }, {} as Record<number, SOPWorkflow[]>);

            // Sort categories: Priority Slugs first, then others alphabetically
            const sortedCategories = [...categories].sort((a, b) => {
              const aIdx = prioritySlugs.indexOf(a.name);
              const bIdx = prioritySlugs.indexOf(b.name);
            ...
            return sortedCategories.map((category) => {
              const categoryWorkflows = grouped[category.id] || [];
              
              return (
                <div key={category.id} className="space-y-6">
                  <div className="flex items-center space-x-4">
                    <h4 data-testid={`category-header-${category.name}`} className="font-label text-[10px] text-primary uppercase tracking-[0.3em] font-bold">
                      {category.display_name}
                    </h4>
                    <div className="h-[1px] flex-1 bg-outline-variant/10"></div>
                    <span className="font-mono text-[9px] text-outline opacity-50">
                      {categoryWorkflows.length} {categoryWorkflows.length === 1 ? 'PROTOCOL' : 'PROTOCOLS'}
                    </span>
                  </div>

                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    <AnimatePresence>
                      {categoryWorkflows.map((workflow, idx) => (
                        <motion.div
                          key={workflow.id}
                        ...
                        >
                          ...
                          <div className="space-y-4 relative z-10">
                            ...
                            <div>
                              <h4 data-testid={`sop-title-${workflow.name}`} className="font-headline text-xl text-on-surface group-hover:text-primary transition-colors">
